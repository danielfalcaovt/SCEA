...
env.config({path:"../config/env/.env"});
...
app.use(express.static(path.join("src/shared/public")));
...
app.post("/api/register", async (req:any,res:any)=>{
  ...
  if (password.length === 0 && username.length === 0) {
    ...
    if (user.rows.length > 0){
      ...
      bcrypt.hash(password.trim(),10,async(err:Error,hashedPassword:string)=>{
        if (err){
          console.log(err);
        }else{
          const user = await db.query("INSERT INTO users(email,password,admin) VALUES($1,$2,$3) RETURNING *",[username,hashedPassword.trim(),"N"]);
